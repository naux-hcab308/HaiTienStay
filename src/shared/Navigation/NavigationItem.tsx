...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  ...
  const renderMainItem = (item: NavItemType) => {
    return (
      <Link
        rel="noopener noreferrer"
        className="inline-flex whitespace-nowrap items-center text-sm xl:text-[15px] font-normal text-neutral-700 dark:text-neutral-300 py-2 px-3 xl:px-4 rounded-full hover:text-neutral-900 hover:bg-neutral-100 dark:hover:bg-neutral-800 dark:hover:text-neutral-200"
        href={item.href || "/"}
      >
        {item.name}
        {item.type && (
